...
function gitSync() {
  ...
  if (!token || !repo) { log("GH_TOKEN / GH_REPO not set — skipping git push (feed written to disk only)."); return; }
  ...
  if (!existsSync(path.join(REPO_DIR, ".git"))) {
    log("cloning repo…");
    execSync(`git clone --depth 1 ${cloneUrl} ${REPO_DIR}`, { stdio: "inherit" });
  } else {
    try { execSync(`git -C ${REPO_DIR} pull --ff-only`, { stdio: "pipe" }); } catch (e) { log("git pull failed (continuing):", e.message); }
  }
  execSync(`git -C ${REPO_DIR} config user.name "${name}"`);
  execSync(`git -C ${REPO_DIR} config user.email "${email}"`);
  execSync(`git -C ${REPO_DIR} add pwa/data/prices.json`);
  let hasChanges = true;
  try { execSync(`git -C ${REPO_DIR} diff --cached --quiet`, { stdio: "pipe" }); hasChanges = false; } catch (e) {}
  if (!hasChanges) { log("no price changes — nothing to push."); return; }
  execSync(`git -C ${REPO_DIR} commit -m "chore: refresh price feed (device)"`, { stdio: "pipe" });
  execSync(`git -C ${REPO_DIR} push origin HEAD`, { stdio: "inherit" });
...
function main() {
  if (ONCE) { runOnce().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); }); return; }
  ...
  cron.schedule(CRON_FULL, () => runOnce().catch(e => console.error(e)), { timezone: "UTC" });
  cron.schedule(CRON_SPECIALS, () => runOnce().catch(e => console.error(e)), { timezone: "UTC" });
